...
dd <- data_tbl %>%
  ...
  mutate(
    nuts = as.factor(nuts),
    is_capital = ifelse(nuts == "SE110", "Capital", "Other"),
...
df_wide[, metro_pct_contrib :=
          100 * (metro - shift(metro, 1L)) /
          shift(national_total, 1L)]
...
df_plot <- df_wide %>%
  select(year, national_pct_growth,
         capital_pct_contrib, metro_pct_contrib, rural_pct_contrib) %>%
  pivot_longer(
...
save_html(
  html_page,
  "C:/Users/annai/Documents/backup_computer_ec/Contratti_lavoro/EU_Temporary_agent/materiali_di_lavoro/mapadomo/mapadomo_v2/SE/website/residential_growth_report.html"
)
