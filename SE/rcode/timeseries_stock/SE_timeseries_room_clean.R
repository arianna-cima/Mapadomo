...
type_map <- c(
  ...
  "2 or more rooms with kitchenette"    = "dwell_3room",  # unsure about this
...
stock_all <- stock[
  ...
  by = .(year, observations, size.of.dwelling, region, room)
]
...
# Reorder columns
setcolorder(stock_combined,
            c("year", "observations", "size.of.dwelling",
              "type.of.building", "region", "value", "room"))
...
stock_final <- stock_long %>%
  ...
  dplyr::select(
    year, type.of.building, value,
    `NUTS 3 CODE`, `LAU CODE`
  ) %>%
  dplyr::rename(nuts3 = `NUTS 3 CODE`) %>%
  mutate(
...
all <- all[
  , .(value = sum(value, na.rm = TRUE)),
  by = .(nuts, year, level, measure, type.of.building)
]
