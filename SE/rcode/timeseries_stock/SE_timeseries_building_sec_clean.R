...
# --- Load dwelling data ---
stock <- fread(file.path(source_stock_path, "SE_secondary.csv"), encoding = "UTF-8")

# --- Reshape data (melt) ---
stock_long <- melt(
  stock,
  id.vars      = c("year", "observations",  "region"),
  measure.vars = "value",
...
agg_nuts3_total <- dt[, .(value = sum(value, na.rm = TRUE)), by = .(nuts3, year)]
agg_nuts3_total[, level := "nuts3"]
setnames(agg_nuts3_total, "nuts3", "nuts")

agg_nuts2_total <- dt[, .(value = sum(value, na.rm = TRUE)), by = .(nuts2, year)]
agg_nuts2_total[, level := "nuts2"]
setnames(agg_nuts2_total, "nuts2", "nuts")

agg_nuts1_total <- dt[, .(value = sum(value, na.rm = TRUE)), by = .(nuts1, year)]
agg_nuts1_total[, level := "nuts1"]
setnames(agg_nuts1_total, "nuts1", "nuts")

agg_nuts0_total <- dt[, .(value = sum(value, na.rm = TRUE)), by = .(nuts0, year)]
agg_nuts0_total[, level := "nuts0"]
setnames(agg_nuts0_total, "nuts0", "nuts")

# --- Aggregate at LAU level ---
agg_lau_total <- dt[, .(value = sum(value, na.rm = TRUE)), by = .(laucode, year)]
...
agg_all_levels$measure <- "bldn_sec"
setnames(agg_all_levels, "measure", "indicator")
agg_all_levels$type.of.building <- "all"


# Save final output
write.csv(
  agg_all_levels,
  file = file.path(output_path, "SE_building_secondary.csv"),
  row.names = FALSE
)
