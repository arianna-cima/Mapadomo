...
pdf_dir <- file.path(base_path,"EE", "sourcedata", "pdf_prices", "house_all")
...
write.csv(
  stock_final,
  file = file.path(source_prices_path, "EE_house_all.csv"),
  row.names = FALSE
)

share <- fread(file.path(source_prices_path, "EE_share_all_existing.csv"), encoding = "UTF-8")

merged_stock_share <- merge(
  stock_final,
  share,
  by = "LAU CODE",
  all.x = TRUE
...
merged_stock_share$price_m2<- merged_stock_share$price_m2*merged_stock_share$avg_share
merged_stock_share[, avg_share := NULL]
merged_stock_share <- merged_stock_share[year > 2006]

write.csv(
  merged_stock_share,
  file = file.path(source_prices_path, "EE_house_existing.csv"),
  row.names = FALSE
)
