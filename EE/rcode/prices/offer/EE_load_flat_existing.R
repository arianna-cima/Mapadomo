...
pdf_dir <- file.path(base_path,"EE", "sourcedata", "pdf_prices", "flat_existing")
...
kv_year$type<-"flat"
kv_year$typev2<-"existing"
kv_year[, price_m2 := ifelse(is.nan(price_m2) | price_m2 == 0, NA, round(price_m2))]

kv_year <- kv_year[kv_year$year %in% c(2023, 2024, 2025)]
...
write.csv(
  stock_final,
  file = file.path(source_prices_path, "EE_flat_existing.csv"),
  row.names = FALSE
)

missing<- stock_final[is.na(stock_final$`NUTS 3 CODE 2021`),]
