...
flat <- fread(file.path(source_prices_path, "EE_price_house_2003_2026_v1.csv"), encoding = "UTF-8")
setDT(flat)
...
valds <- unique(flat[, .(vald_clean, vald, `LAU CODE`, `NUTS 3 CODE 2021`, `NUTS 3 CODE 2016`, type)])


panel <- CJ(year = all_years, vald_clean = valds$vald_clean)
panel <- merge(panel, valds, by = "vald_clean", all.x = TRUE)

panel <- merge(panel, flat, by = c("vald_clean","vald","year","type","LAU CODE","NUTS 3 CODE 2021","NUTS 3 CODE 2016"), all.x = TRUE)
...
panel <- merge(panel, nuts3_level, by = c("NUTS 3 CODE 2021","year"), all.x = TRUE)
...
panel[, median_interp := zoo::na.approx(median_price_m2, x = year, na.rm = FALSE), by = vald_clean]

panel[, average_interp := zoo::na.approx(average_price_m2, x = year, na.rm = FALSE),by = vald_clean]

# ============================
# 5. Initialize final variables
# ============================

panel[, `:=`(median_price_final = median_price_m2, average_price_final = average_price_m2)]


panel[is.na(median_price_final), median_price_final := median_interp]

panel[is.na(average_price_final),average_price_final := average_interp]
...
panel[is.na(median_price_final),median_price_final := nat_median]

panel[is.na(average_price_final),average_price_final := nat_average]

# ============================
# 9. Diagnostics
# ============================

cat("Missing median_price_final:",panel[, sum(is.na(median_price_final))], "\n")

cat("Missing average_price_final:",panel[, sum(is.na(average_price_final))], "\n")



panel<-panel[,c(1:8,18:19)]
...
write.csv(
  panel,
  file = file.path(source_prices_path, "EE_price_house_2003_2026.csv"),
  row.names = FALSE
)
