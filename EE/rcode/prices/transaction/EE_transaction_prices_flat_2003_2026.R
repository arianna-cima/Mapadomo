...
#Toila vald imputed with mean "Jõhvi vald", "Alutaguse vald", "Lüganuse vald" and Ruhnu vald imputed with Kihnu vald
# ================================
# Read and merge Estonian house price Excel files
# Robust to "***" and mixed column types
# ================================
...
data_path <- file.path(
  base_path,
  "EE/sourcedata/transaction_prices"
)

# List all relevant Excel files
files <- list.files(
  path = data_path,
  pattern = "Real_property_price_statistics_res_trans_.*\\.xlsx$",
  full.names = TRUE
)
...
dd <- dd %>%
  fill(year, vald, .direction = "down")

# ---- 4. Clean vald names ----
dd <- dd %>%
  mutate(
    vald = str_squish(vald)
  )
...
dd <- dd[!grepl("Data|Source", as.character(dd$year)), ]
...
source_vald <- "Kihnu vald"
target_vald <- "Ruhnu vald"

vars_to_impute <- c("number", "avg_area_m2", "total_price_eur",
                    "min_price_eur", "max_price_eur")

# -------------------------------
# 1. Create Ruhnu rows (from Kihnu structure)
# -------------------------------

ruhnu_template <- unique(
  dd[vald == source_vald, .(year, area_bin_m2)]
)
ruhnu_template[, vald := target_vald]

setkey(dd, year, area_bin_m2, vald)
setkey(ruhnu_template, year, area_bin_m2, vald)

dd <- rbindlist(
  list(dd, ruhnu_template[!dd]),
  use.names = TRUE, fill = TRUE
)

# -------------------------------
# 2. Copy Kihnu values into Ruhnu
# -------------------------------
...
for (v in vars_to_impute) {
  dd[
    vald == target_vald,
    (v) := dd[
      vald == source_vald & year == .BY$year & area_bin_m2 == .BY$area_bin_m2,
      get(v)
    ],
    by = .(year, area_bin_m2)
  ]
...
dd <- dd[area_bin_m2 == "TOTAL"]
dd <-dd[, .(vald, year, type, number, average_price_m2, median_price_m2)]
...
write.csv(
  final_dt,
  file = file.path(source_prices_path, "EE_price_flat_2003_2026_v1.csv"),
  row.names = FALSE
)
